...
class simple_def_reg(object):
    def __init__(self, reg, bit_width):
        self.reg = reg
...
class pruning_database(object):
    ...
    def _add_to_pc_map(self, pc, pilot):
    ...
    def print_pruning_db(self,filename):
        ...
        output.write('pc def_pc do_inject src_regs mem_src_regs dest_reg is_mem pilot max_bits\n')
        for pc in self.pc_list:
            for pc_obj in self.pc_map[pc]:
                ...
                output.write('%s %s %r %s %s %s %r %s %d\n' % (pc, def_pc,
